...
public class AVLTree {
     ...
     * 中序遍历
     */
    public void midOrderTraverse(BaseNode node){
        if (node == null) {
            return;
        }
     ...
     *
     */
    public void afterOrderTraverse(BaseNode node){
        if (node == null) {
            return;
        }
    ...
    public void preOrder(BaseNode biTree){
        System.out.printf(biTree.key+"");
        BaseNode leftTree=biTree.left;
        if(leftTree!=null){
            preOrder(leftTree);
        }
        BaseNode rightTree=biTree.right;
        if(rightTree!=null){
            preOrder(rightTree);
        }
    }
    public void inOrderTraversal(BaseNode node){
        if(node == null){
            return;
        }else{
    ...
    public void postOrder(BaseNode biTree){
        BaseNode leftTree = biTree.left;
        if (leftTree != null) {
            postOrder(leftTree);
        }
    ...
    public List<List<Object>> zigzagLevelOrder(BaseNode root) {
    ...
    public void dfs(BaseNode node, List<List<Object>> list, int level) {
        if (node == null) {
            return;
        }
        if (list.size() <= level) {
        ...
        List<Object> curList = list.get(level);
        if ((level & 1) == 0) {
            curList.add((Object) node.key);
        } else {
            curList.add(0, (Object) node.key);
        }
